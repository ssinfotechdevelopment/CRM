...
const courseAPI = {
  getAllCourses: async () => {
    try {
      ...
      const response = await fetch('https://crm-backned-v1.onrender.com/api/courses', {
        method: 'GET',
        headers: {
  ...
  updateCourse: async (courseId, courseData) => {
    const token = localStorage.getItem('adminToken');
    const response = await fetch(`https://crm-backned-v1.onrender.com/api/courses/${courseId}`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
  ...
  addStudent: async (courseId, studentData) => {
    const token = localStorage.getItem('adminToken');
    const response = await fetch(`https://crm-backned-v1.onrender.com/api/courses/${courseId}/students`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
  ...
  updateStudent: async (courseId, studentId, studentData) => {
    const token = localStorage.getItem('adminToken');
    const response = await fetch(`https://crm-backned-v1.onrender.com/api/courses/${courseId}/students/${studentId}`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
  ...
  updateReminder: async (courseId, studentId, reminderMessage) => {
    const token = localStorage.getItem('adminToken');
    const response = await fetch(`https://crm-backned-v1.onrender.com/api/courses/${courseId}/students/${studentId}/reminder`, {
      method: 'PATCH',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
      ...
      <div className="bg-red-50 border border-red-200 rounded-lg p-6">
        ...
        {error.includes('server') && (
          <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded text-sm text-yellow-800">
            ...
            <p>• Verify API endpoint: https://crm-backned-v1.onrender.com/api/courses</p>
